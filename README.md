...
配布版は無料で利用できます。標準モデル、FFmpeg、字幕用フォント、ライセンス表記などを同梱した、すぐ使えるパッケージとして公開します。
...
Lumimi は GNU GPL v3.0 の条件に従って、利用・改変・再配布が可能です。
作者表示、著作権表示、ライセンス表記を削除したり、ご自身が一から制作したものと誤認させる形で配布することはお控えください。
...
追加モデルのダウンロード時、および GitHub など外部配布サービス利用時にはインターネット接続を使用します。
...
Release builds are distributed free of charge and provide a ready-to-use package including the standard model, FFmpeg, subtitle fonts, and license notices.
...
- Windows release builds are distributed free of charge
...
Lumimi may be used, modified, and redistributed under the terms of GNU GPL v3.0.
Please do not remove author, copyright, or license notices, or distribute it in a way that falsely suggests you created the original project from scratch.
...
Internet access is used when downloading optional models or when using external distribution services such as GitHub.
